...
!  private :: soilwater_head_form
  ! Finn initial starts the Hydro-thermal Couple module at 02.27.2025
  private :: soil_freeze_thaw_couple
  ! First added the couple subroutine
    ...
    associate(&
      ...
      do fc = 1, num_hydrologyc
         ...
         do ! continuous do loop with exit clause
            ...
            ! Finn 
            call soil_freeze_thaw_couple(c, nlayers, temperature_inst, waterstatebulk_inst, soilstate_inst, param=.TRUE.)
            ...
            ! Finn added for soil freeze thaw couple subroutine
            !call soil_freeze_thaw_couple(c, nlayers, temperature_inst, waterstatebulk_inst, soilstate_inst, param=.TRUE.) 
            ...
            ! Finn added for soil freeze thaw couple subroutine
            call soil_freeze_thaw_couple(c, nlayers, temperature_inst, waterstatebulk_inst, soilstate_inst, param=.FALSE.)
...
!#7.5
! Finn added soil_freeze_thaw_couple subroutine
  !-----------------------------------------------------------------------   
  subroutine soil_freeze_thaw_couple(c, nlayers, temperature_inst, waterstatebulk_inst, soilstate_inst, param)
    !
    ! For soil freezing and thawing couple processes, and using SWC, SFC
    ! Clapeyron Equation for calculate soil ice content and update 
    ! soil matrix suction psi_w and soil liquid water theta_w
    !
    !USES:
    use decompMod            , only : bounds_type
    use ColumnType           , only : col
    ...
    use WaterStateBulkType   , only : waterstatebulk_type
    use TemperatureType      , only : temperature_type
    use clm_varcon           , only : tfrz, hfus, grav, denh2o, denice
    use clm_varctl           , only : iulog
    !
    ! !ARGUMENTS:
    integer                  , intent(in)    :: c                    ! column index
    integer                  , intent(in)    :: nlayers              ! lower boundary index
    logical                  , intent(in)    :: param                ! paramter for control whether update h2osoi_liq
    type(temperature_type)   , intent(in)    :: temperature_inst
    type(waterstatebulk_type), intent(inout) :: waterstatebulk_inst
    type(soilstate_type)     , intent(inout) :: soilstate_inst
    ...
    integer  :: pi                                                    ! patch index
    real(r8) :: dtime                                                 ! land model time step (sec)
    real(r8) :: vol_liq(1:nlayers)                                    ! liquid volumetric water content
    real(r8) :: vol_liq0(1:nlayers)
    real(r8) :: h2oliq(1:nlayers)                                     ! water content 
    real(r8) :: h2otot(1:nlayers)
    real(r8) :: vol_tot(1:nlayers)
    real(r8) :: vol_tot0(1:nlayers)
    real(r8) :: vol_water(1:nlayers)
    real(r8) :: mass0(1:nlayers)
    real(r8) :: vol_ice(1:nlayers)                                    ! volumetric ice content
    real(r8) :: vol_ice0(1:nlayers)
    real(r8) :: s_node
    real(r8) :: smp_tf
    real(r8) :: delta_vol_ice(1:nlayers)
    real(r8) :: eff_porosity(1:nlayers)                               ! effeicient porosity
    real(r8) :: T_freez(1:nlayers)                                    ! freezing point using Clapeyron Equation
    real(r8) :: psi_ori(1:nlayers)
    real(r8) :: psi_w0(1:nlayers)                                     ! soil matrix suction put in SWC & SFC [mm]
    real(r8) :: psi_w1(1:nlayers)                                     ! psi_w1 = psi_w0 + psi_freez [using generalized Clapeyron Equation]
    real(r8) :: mm_to_m = 1e-3_r8                                     ! ratio for soil matrix suction's unit change [mm] to [m]
    real(r8) :: theta_w0(1:nlayers)                                   ! liquid volumetric water content if there is freezing thawing process
    real(r8) :: theta_w1(1:nlayers)                                   ! liquid volumetric water content if there is freezing thawing process
    real(r8) :: theta_ice(1:nlayers)
    real(r8) :: heaviside                                             ! Heaviside function H(x)=0 if x<0 ; H(x)=1 if x>=0
    real(r8) :: SWC_vol_r = 0._r8                                     ! volumetic residual water in SWC 
    real(r8) :: SWC_a = 0.01_r8                                       ! Parameter a in SWC
    real(r8) :: SWC_n = 1.17_r8                                       ! Parameter N in SWC
    character(len=32)  :: subname = 'soil_freeze_thaw_couple'         ! subroutine name 
    !-----------------------------------------------------------------------   
    ...
    associate(                                                            &
         ...
         sucsat           =>    soilstate_inst%sucsat_col               , & ! Input:[real(r8) (:,:) ] minimum soil suction (mm)              
         watsat           =>    soilstate_inst%watsat_col               , & ! Input:[real(r8) (:,:) ] volumetric soil water at saturation (porosity)
         smp_l            =>    soilstate_inst%smp_l_col                , & ! Input&Output: [real(r8) (:,:) ]  soil matrix potential [mm]
         T_f_freeze       =>    soilstate_inst%T_f_col                  , & ! Output:[real(r8) (:,:) ] soil freeze temperature [k]
         vol_ice_int      =>    soilstate_inst%vol_ice_int_col          , &
         vol_tot_col      =>    soilstate_inst%vol_tot_col              , &
         smp_w0           =>    soilstate_inst%smp_w0_col               , &
         smp_w1           =>    soilstate_inst%smp_w1_col               , &
         vol_liq_w0       =>    soilstate_inst%vol_liq_w0_col           , &
         vol_liq_w1       =>    soilstate_inst%vol_liq_w1_col           , &
         h2osoi_ice_pre   =>    soilstate_inst%h2osoi_ice_pre_col       , & 
         ...
         volliq_noph      =>    soilstate_inst%volliq_noph_col          , & 
         t_soisno         =>    temperature_inst%t_soisno_col             & ! Output: [real(r8) (:,:) ] soil temperature [K] 
         )  ! end associate statement    
      
      ! calculated volumetric soil water and ice content
      do j = 1, nlayers
         !vol_tot0(j) = vol_tot_col(c,j)
         vol_ice0(j) = min(watsat(c,j), h2osoi_ice(c,j)/(dz(c,j)*denice))
         vol_ice0(j) = max(0._r8, vol_ice0(j))
         eff_porosity(j) = watsat(c,j)-vol_ice0(j)
         vol_liq0(j) = min(eff_porosity(j), h2osoi_liq(c,j)/(dz(c,j)*denh2o))
         vol_liq0(j) = max(vol_liq0(j),1.0e-6_r8)
         mass0(j)    = h2osoi_ice(c,j) + h2osoi_liq(c,j)
         vol_tot0(j) = vol_ice0(j)*(denice/denh2o) + vol_liq0(j)
         psi_ori(j)  = smp_l(c,j)
      end do
     
      ! input soil water, if param = TRUE, vol_water = vol_tot, else , vol_water
      ! = vol_liq0
      do j = 1, nlayers
         !if (param) then
            vol_water(j) = vol_tot0(j)
         !else 
         !   vol_water(j) = vol_liq0(j)
         !end if
            h2osoi_ice_pre(c,j) = vol_ice0(j)*denice*dz(c,j)
      end do      

      ! calculate T_freez using Clayperon Equation
      do j = 1, nlayers
         T_freez(j) = tfrz
         if (t_soisno(c,j) < tfrz) then
           ! T_freez
           s_node = vol_water(j)/watsat(c,j)
           s_node = max(0.01_r8, s_node)
           s_node = min(s_node, 1._r8) 
           smp_tf = -sucsat(c,j)*s_node**(-bsw(c,j))
           !T_freez(j) = 1000._r8*hfus*tfrz/(1000._r8*hfus-grav*smp_tf)
           T_freez(j) = tfrz + ((grav*tfrz/hfus)*(smp_tf*mm_to_m))
           T_freez(j) = max(269.15_r8, T_freez(j))
           T_freez(j) = min(T_freez(j), 273.15_r8)
         endif
         T_f_freeze(c,j) = T_freez(j)
      end do
 
      ! Using generalized Clapeyron Equation (Dall'Amico, 2010) to calculate
      ! psi_w1 for theta_i and mass conservation
      do j = 1, nlayers
         ! Initialization for psi & theta
         s_node = vol_water(j)/watsat(c,j)
         s_node = max(0.01_r8, s_node)
         s_node = min(s_node, 1._r8)
         psi_w0(j) = (-sucsat(c,j))*s_node**(-bsw(c,j))                              !using SWC -> psi_w0 [mm]                                                                 
         !psi_w0(j) = smp_l(c,j) 
         !theta_w0(j)  = watsat(c,j)*(psi_w0(j)/(sucsat(c,j)*mm_to_m*-1._r8))**(-1._r8/bsw(c,j))!using reversed SWC -> theta_w0 which equal to vol_tot
         !theta_w0(j) = min(theta_w0(j), watsat(c,j)-vol_ice0(j))
         !theta_w0(j) = max(1.0e-6_r8, theta_w0(j))
         theta_w0(j) = vol_water(j)
         psi_w1(j) = psi_w0(j)                                                          !initilize psi_w1 = psi_w0   
         theta_w1(j) = vol_water(j)                                                     !initilize theta_w1 = theta_w0 

         ! Using psi_w0 & T_freez for psi_w1
         ! first to set the heaviside value for Heaviside Function
         if (t_soisno(c,j) <= T_freez(j)) then
           ! calculated psi_w1
           !if (param) then
             psi_w1(j) = psi_w0(j) + ((hfus/grav/tfrz)*(t_soisno(c,j)-T_freez(j))*1e3_r8)  ![mm]
           !else
           !  psi_w1(j) = psi_w0(j) + ((hfus/grav/tfrz)*(T_freez(j)-t_soisno(c,j))*1e3_r8)  ![mm]
           ! using psi_w1 to calculate theta_w1
           theta_w1(j) = watsat(c,j)*(psi_w1(j)/(-sucsat(c,j)))**(-1._r8/bsw(c,j)) 
           theta_w1(j) = min(theta_w1(j), vol_water(j))
           ! Van Genuchten SWC 
           !theta_w1(j) = SWC_vol_r + (watsat(c,j) - SWC_vol_r)*((1._r8+(SWC_a*(-1._r8*(psi_w1(j)))**SWC_n))**(-1._r8*(1._r8-(1._r8/SWC_n))))
           !end if
         end if
         smp_w0(c,j) = psi_w0(j)
         vol_liq_w0(c,j) = theta_w0(j)
         smp_w1(c,j) = psi_w1(j)
         vol_liq_w1(c,j) = theta_w1(j)
      end do

      ! update vol_ice so update h2osoi_liq & h2osoi_ice and smp_l

      do j = 1, nlayers
         if (t_soisno(c,j) < T_freez(j)) then
           theta_w1(j) = min(vol_liq0(j),theta_w1(j))
           !psi_w1(j) = min(psi_w1(j), psi_ori(j))
           !smp_l(c,j) = psi_w1(j)
         elseif (t_soisno(c,j) > T_freez(j)) then
           theta_w1(j) = min(theta_w1(j), vol_tot0(j))
           !smp_l(c,j) = psi_w1(j)
         end if
         if (param) then
           vol_ice(j) = (vol_water(j)-theta_w1(j))*(denh2o/denice)
           h2osoi_ice(c,j) = vol_ice(j)*dz(c,j)*denice
           vol_liq(j) = theta_w1(j)
           h2osoi_liq(c,j) = vol_liq(j)*dz(c,j)*denh2o
         else
           vol_ice(j) = (vol_water(j)-theta_w1(j))*(denh2o/denice)
           h2osoi_ice(c,j) = vol_ice(j)*dz(c,j)*denice
           vol_liq(j) = theta_w1(j)
         ...
         if (vol_ice(j) > vol_ice0(j) .and. t_soisno(c,j) < T_freez(j)) then
           vol_liq(j) = min(vol_liq(j), vol_liq0(j))
         end if 
      end do

    end associate
  end subroutine soil_freeze_thaw_couple
