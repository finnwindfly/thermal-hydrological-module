module SoilStateType
  ...
  type, public :: soilstate_type
     ...
     ! Finn for T_freeze
     real(r8), pointer :: T_f_col              (:,:)
     real(r8), pointer :: volliq_noph_col      (:,:)
     real(r8), pointer :: vol_ice_int_col      (:,:)
     real(r8), pointer :: vol_tot_col          (:,:) 
     real(r8), pointer :: smp_w1_col           (:,:)
     real(r8), pointer :: vol_liq_w1_col       (:,:)
     real(r8), pointer :: smp_w0_col           (:,:)
     real(r8), pointer :: vol_liq_w0_col       (:,:)
     real(r8), pointer :: h2osoi_ice_pre_col   (:,:)
...
contains
  ...
  subroutine InitAllocate(this, bounds)
    ...
    ! Finn for T_freeze
    allocate(this%T_f_col              (begc:endc,nlevsoi))             ; this%T_f_col              (:,:) = nan
    allocate(this%volliq_noph_col      (begc:endc,nlevsoi))             ; this%volliq_noph_col      (:,:) = nan
    allocate(this%vol_ice_int_col      (begc:endc,nlevsoi))             ; this%vol_ice_int_col      (:,:) = nan
    allocate(this%vol_tot_col          (begc:endc,nlevsoi))             ; this%vol_tot_col          (:,:) = nan
    allocate(this%smp_w1_col           (begc:endc,nlevsoi))             ; this%smp_w1_col           (:,:) = nan
    allocate(this%vol_liq_w1_col       (begc:endc,nlevsoi))             ; this%vol_liq_w1_col       (:,:) = nan
    allocate(this%smp_w0_col           (begc:endc,nlevsoi))             ; this%smp_w0_col           (:,:) = nan
    allocate(this%vol_liq_w0_col       (begc:endc,nlevsoi))             ; this%vol_liq_w0_col       (:,:) = nan
    allocate(this%h2osoi_ice_pre_col   (begc:endc,nlevsoi))             ; this%h2osoi_ice_pre_col   (:,:) = nan
  ...
  subroutine InitHistory(this, bounds)
    ...
    ! Finn for T_f
    this%T_f_col(begc:endc,:) = spval
    call hist_addfld2d (fname='TFRE', units='K', type2d='levsoi', &
            avgflag='A', long_name='Soil Freeze Point', &
            ptr_col=this%T_f_col)

    this%volliq_noph_col(begc:endc,:) = spval
    call hist_addfld2d (fname='H2OLIQNOPH', units='m^3/m^3', type2d='levsoi', &
            avgflag='A', long_name='soil volumetric water content with no phase', &
            ptr_col=this%volliq_noph_col)

    this%vol_ice_int_col(begc:endc,:) = spval
    call hist_addfld2d (fname='H2OSOIICE', units='m^3/m^3', type2d='levsoi', &
            avgflag='A', long_name='intermediate volumetric soil ice content', &
            ptr_col=this%vol_ice_int_col)

    this%vol_tot_col(begc:endc,:) = spval
    call hist_addfld2d (fname='H2OSOITOT', units='m^3/m^3', type2d='levsoi', &
            avgflag='A', long_name='total volumetric soil water content(liquid+ice)', &
            ptr_col=this%vol_tot_col)

    this%smp_w1_col(begc:endc,:) = spval
    call hist_addfld2d (fname='SMPW1', units='m', type2d='levsoi', &
            avgflag='A', long_name='intermediate soil matrix potenial', &
            ptr_col=this%smp_w1_col)

    this%vol_liq_w1_col(begc:endc,:) = spval
    call hist_addfld2d (fname='H2OSOIW1', units='m^3/m^3', type2d='levsoi', &
            avgflag='A', long_name='intermediate volumetric soil liquid', &
            ptr_col=this%vol_liq_w1_col)

    this%smp_w0_col(begc:endc,:) = spval
    call hist_addfld2d (fname='SMPW0', units='m', type2d='levsoi', &
            avgflag='A', long_name='intermediate soil matrix potenial', &
            ptr_col=this%smp_w0_col)

    this%vol_liq_w0_col(begc:endc,:) = spval
    call hist_addfld2d (fname='H2OSOIW0', units='m^3/m^3', type2d='levsoi', &
            avgflag='A', long_name='intermediate volumetric soil liquid', &
            ptr_col=this%vol_liq_w0_col)

    this%h2osoi_ice_pre_col(begc:endc,:) = spval
    call hist_addfld2d (fname='SOIICEPRE', units='kg/m^-2', type2d='levsoi', &
            avgflag='A', long_name='soil ice content at previous time step', &
            ptr_col=this%h2osoi_ice_pre_col)
