...
contains
  ...
  subroutine Phasechange_beta (bounds, num_nolakec, filter_nolakec, dhsdT, &
    ...
    use clm_varpar       , only : nlevsno, nlevgrnd,nlevurb
    use clm_varctl       , only : iulog
    use clm_varcon       , only : tfrz, hfus, grav, denice ! Finn
    ...
    real(r8) :: vol_ice_pre(bounds%begc:bounds%endc,1:nlevgrnd)
    !-----------------------------------------------------------------------
    ...
    associate(                                                        & 
         ...
         t_soisno         =>    temperature_inst%t_soisno_col       , & ! Output: [real(r8) (:,:) ] soil temperature [K]             
         h2osoi_ice_pre   =>    soilstate_inst%h2osoi_ice_pre_col   , & ! Finn previous time step's h2osoi_ice 
         csol             =>    soilstate_inst%csol_col               & ! Finn 
         )
      ...
      ! Finn for comment for soil freeze thaw process
      !do j = -nlevsno+1,nlevgrnd       ! all layers
      do j = -nlevsno+1,0               ! just for snow layers
         do fc = 1,num_nolakec
            c = filter_nolakec(fc)
      ...
      ! Finn for zero change in soil layers
      do j = 1,nlevgrnd-5               ! just for snow layers
         do fc = 1,num_nolakec
            c = filter_nolakec(fc)
            ...
            if ((col%itype(c) /= icol_sunwall .and. col%itype(c) /= icol_shadewall &
               ...
               if (t_soisno(c,j) <= tfrz .or. (t_soisno(c,j) > tfrz .and. wice0(c,j) > 0._r8)) then  
                 !vol_ice_pre(c,j) = h2osoi_ice_pre(c,j) / dz(c,j) / denice
                 !vol_ice(c,j)     = wice0(c,j) / dz(c,j) / denice
                 hm(c,j) = hfus * (wice0(c,j)-h2osoi_ice_pre(c,j)) / dtime
                 if (j == 1) then
                   !hm(c,j) = (1.0_r8 - frac_sno_eff(c) - frac_h2osfc(c))*hm(c,j)
                   hm(c,j) = 0._r8
                 else                  
                   hm(c,j) = hm(c,j)
                 endif
                 !heatr = hm(c,j) - hfus*(wice0(c,j)-h2osoi_ice_pre(c,j))/dtime

                 !heatr = 0._r8
                 !vol_ice_pre(c,j) = h2osoi_ice_pre(c,j) / dz(c,j) / denice
                 !vol_ice(c,j)     = wice0(c,j) / dz(c,j) / denice
                 !heatr = hfus * denice * (vol_ice(c,j)-vol_ice_pre(c,j)) / dtime
                 h2osoi_ice(c,j) = wice0(c,j)
                 if (j == 1) then
                   t_soisno(c,j) = t_soisno(c,j)
                   !                /(1._r8-(1.0_r8 - frac_sno_eff(c) - frac_h2osfc(c))*fact(c,j)*hm(c,j))
                 else
                   t_soisno(c,j)   = t_soisno(c,j) + (hm(c,j) * dtime / csol(c,j) / dz(c,j))
                 end if
               end if
            end if
